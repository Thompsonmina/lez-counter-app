...
```
lez_counter/
├── lez_counter_core/    # Shared types (used by guest + host)
│   └── src/lib.rs
├── methods/
│   └── guest/            # RISC Zero guest program (runs on-chain)
│       └── src/bin/lez_counter.rs
├── examples/             # CLI tools
│   └── src/bin/
│       ├── generate_idl.rs    # One-liner IDL generator
│       └── lez_counter_cli.rs # Three-line CLI wrapper
├── spel.toml                         # SPEL CLI config (IDL and binary paths)
├── Makefile
└── lez_counter-idl.json       # Auto-generated IDL
```
